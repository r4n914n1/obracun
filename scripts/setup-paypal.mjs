/**
 * Kreira PayPal product, billing planove i webhook preko REST API-ja.
 *
 * Upotreba (Sandbox):
...
const PLANS = [
  { id: 'lite-1m', name: 'Lite', priceEur: 1, months: 1, limit: 100 },
...
function upsertEnvFile(planIds, webhookId, clientId, clientSecret, mode) {
  ...
  // Secrets must NOT live in functions/.env — Firebase loads it and conflicts
  // with defineSecret. Keep them in .env.secrets for local scripts only.
  map.delete('PAYPAL_CLIENT_ID')
  map.delete('PAYPAL_CLIENT_SECRET')
  map.delete('PAYPAL_WEBHOOK_ID')
  ...
  const header = [
    ...
    `# Webhook: ${webhookUrl}`,
    '# Secrets: functions/.env.secrets (PAYPAL_CLIENT_* / WEBHOOK_ID)',
    '',
  ]
  ...
  if (clientId || clientSecret || webhookId) {
    const secretMap = loadEnvFile(secretsPath)
    if (clientId) secretMap.PAYPAL_CLIENT_ID = clientId
    if (clientSecret) secretMap.PAYPAL_CLIENT_SECRET = clientSecret
    if (webhookId) secretMap.PAYPAL_WEBHOOK_ID = webhookId
    const secretBody = [
      '# Local only — Firebase Secret Manager holds production copies',
      '# Do not put these keys in functions/.env (deploy conflict)',
      '',
      ...Object.entries(secretMap).map(([k, v]) => `${k}=${v}`),
      '',
    ].join('\n')
    fs.writeFileSync(secretsPath, secretBody, 'utf8')
    console.log(`Secrets sačuvani u ${secretsPath}`)
  }
}
